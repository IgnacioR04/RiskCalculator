          ...
          <Card>
            ...
            <div className="row" style={{ alignItems: 'flex-end', gap: 18, marginTop: 7 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 1, paddingBottom: 5 }}>
                <Figure size="result" className={view.totalUnrealizedPnl.gte(0) ? 'positive' : 'negative'}>
                  <span style={{ fontSize: 19 }}>
                    {view.totalUnrealizedPnl.gte(0) ? '+' : ''}
                    {formatMoney(view.totalUnrealizedPnl, displayCurrency)}
                  </span>
                </Figure>
                <span className="meta">
                  {view.simpleReturnPct !== null
                    ? `${view.simpleReturnPct.gte(0) ? '+' : ''}${formatPct(view.simpleReturnPct)} desde tu primera compra`
                    : 'sin capital aportado registrado'}
                </span>
              </div>
            </div>

            <div className="kpi-row mt-4">
              <Kpi label="Capital aportado" hint="Suma de lo que has puesto, convertido a tu divisa.">
                <Money value={view.totalCost} currency={displayCurrency} size="kpi" />
              </Kpi>
              <Kpi
              ...
              <Kpi label="Rentabilidad simple" hint="Valor actual frente al capital aportado.">
                {view.simpleReturnPct !== null ? (
                  <SignedValue formatted={formatPct(view.simpleReturnPct)} sign={sign(view.simpleReturnPct)} />
                ) : (
                  '—'
                )}
              </Kpi>
